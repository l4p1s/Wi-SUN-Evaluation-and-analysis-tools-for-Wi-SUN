        ...
        with open(output_file, 'w') as file:
            ...
            for i in range(0, len(data), chunk_size):
                ...
                for attempt in range(3):  # 最大3回再送
                    writer.write(f"udps fe80::21d:129f:35c5:2eba {message}\n".encode('utf-8'))
...
async def wait_for_response(reader, chunk_size):
    """レスポンスを非同期で待つ"""
    response = b""
    while b"udpsd <fe80::21d:129f:35c5:2eba>" not in response:
        response += await reader.read(chunk_size + 100)
    return response.decode('utf-8', errors='ignore')

try:
    asyncio.run(send_data(port="/dev/ttyUSB1", baudrate=115200, chunk_size=chunk_size, file_suffix=file_suffix, data=data))
except KeyboardInterrupt:
    print("プログラムを終了します")
    print("Serial connection closed.")
    exit()